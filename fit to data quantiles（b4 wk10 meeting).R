lattice=function(m,c,t=50,tby=1,n=1000){ #m is the index of the parameter, c is the output of myMCMC(),n is the number to sample from c, the posterior distribution, this function returns a lattice of values of data generated from different values of parameter sampled from the posterior c.
	...
	for (j in 1:n){
		...
		if (m==1){
			for (i in 1:((t/tby)+1)){   
	    u[i]=deterministic(i1=d[j])[i] # try c7 first
	   }
		}
		if (m==2){
			for (i in 1:((t/tby)+1)){   
	    u[i]=deterministic(i2=d[j])[i] # try c7 first
	   }
		}
		if (m==3){
			for (i in 1:((t/tby)+1)){   
	    u[i]=deterministic(i3=d[j])[i] # try c7 first
	   }
		}
		if (m==4){
			for (i in 1:((t/tby)+1)){   
	    u[i]=deterministic(i4=d[j])[i] # try c7 first
	   }
		}
		if (m==5){
			for (i in 1:((t/tby)+1)){   
		...
		if (m==13){
			for (i in 1:((t/tby)+1)){   
	    u[i]=deterministic(c8=d[j])[i] # try c7 first
	   }
		}		
	 	   w=rbind(w,u)
	}
