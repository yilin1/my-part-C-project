lognormallk=function(a1,data,tby=1,erv1=1){  #this is calculating the likelihood given the real data: data, 
  ...
  #a1[14] is the scaling factor for standard normal in the error term.
  ...
  for (tmpts in (data$Month)) {
    ...
    lp=dlnorm(observed, meanlog=(log(predicted))-((a1[14])^2)/2, sdlog = a1[14], log = TRUE)
    #lp=dnorm(observed,predicted,sd=sqrt((a1[14]^2)*erv1),log=TRUE) 
    
    l=l+lp
    #print(cbind(predicted,log(predicted),observed))
